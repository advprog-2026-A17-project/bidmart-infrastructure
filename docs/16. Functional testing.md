...
Jalankan frontend: `cd ../bidmart-frontend && npm run dev` (http://localhost:5173).
...
Requires RabbitMQ, PostgreSQL, gateway, wallet, catalogue, auction (Rust), order/notification, and auth. Untuk cek UI, jalankan Vite di `bidmart-frontend` (port 5173).
